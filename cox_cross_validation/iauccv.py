...
class IaucCV(KFold):
    ...
    def split(self, X, y, *args):
        ...
        used.append(maxx)
        # Also pick one of the smallest
        minn = np.random.choice(np.where(y.f1 == y.f1.min())[0])
        used.append(minn)

        # Condition 2.
        # Obeserved events before and after last iAUC evaluation point to test set
        ...
        # There is a chance that there are less than self.n_splits samples
        # with an event before t=2, in this special case, sample with replacement
        # In the dataset used in this project, this is not the case, but it might be in others
        if np.setdiff1d(min_ind, used).shape[0] < self.n_splits:
            min_tests = np.random.choice(np.setdiff1d(min_ind, used), self.n_splits, replace=True)
        else:
            min_tests = np.random.choice(np.setdiff1d(min_ind, used), self.n_splits, replace=False)
        used.extend(min_tests)

        # There will always be enough samples past t=(75th percentile) by definition
